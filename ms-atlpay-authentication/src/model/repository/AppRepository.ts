...
export class AppRepository {
   ...
   * @return{Promise<any[]>}
   */
  public async pagination(page:number=1, size: number=10) {
    page = Number(page);
    size = Number(size);
   ...
   * @return{Promise<any[]>}
   */
  public async find(attributes:string='_'+this._primaryKeys[0],
      order:string='ASC'): Promise<any[]> {
    const repository: Repository<any> = await this.loadRepository();
    const relations = this.getRelations();
    return await repository.find({relations, order: {[attributes]: order}});
  }
  /**
   ...
   * @param{string[]} blockAttributes
   * @return{any}
   */
  private convertToJson(object: any, blockAttributes:string[] = []):any {
    ...
    return this.deleteAttirutes(jsonObject, blockAttributes);
  }
   ...
   */
  public camelToSnakecase(key: string): string {
   ...
   * @param{String[]} blockAttributes - Block Attributes List
   * @return{any}
   */
  public toJson(entities:any, blockAttributes:string[] =
  ['modified_on', 'created_on']): any {
    const convertedEntities : any[]=[];
    if (Array.isArray(entities)) {
      entities.forEach((entity)=>{
        if (typeof entities==='object') {
          entity=this.convertToJson(entity, blockAttributes);
        }
      ...
      if (typeof entities==='object') {
        entities = this.convertToJson(entities, blockAttributes);
      }
      return entities;
    }
  }
  /**
   * Delete attributes of json
   * @param{any} entity
   * @param{Array<string>} blockAttributes
   * @return{any}
   */
  public deleteAttirutes(entity: any, blockAttributes: Array<string>):any {
