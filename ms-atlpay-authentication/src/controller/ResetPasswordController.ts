 ...
 */
export class ResetPasswordController extends AppController {
  /**
   * Database name
   * @var{string}
   */
  ...
  public checkResetToken = async (
    ...
    try {
      ...
      if (isvalidToken) {
        ...
        throw new PasswordResetLinkExpireException(
            resetPasswordLinkExpired);
      }
    } catch (err) {
      next(err);
