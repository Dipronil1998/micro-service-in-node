...
import {cleanEnv, host, str, port, makeValidator, num, url} from 'envalid';
...
const env = cleanEnv(process.env, {
  NODE_ENV: str({choices: ['production', 'development', 'test', 'local']}),
  DB_TYPE: str(),
  DB_HOSTNAME: host(),
  DB_PORT: port(),
  DB_USER: str(),
...
export const AUTH_BASE_URL : string = env.AUTH_BASE_URL;
...
export const passwordHashKey: number = Number(process.env.PASSWORD_HASH_KEY);
export const pinHashKey: number = Number(process.env.PIN_HASH_KEY);
...
export const merchantCreatedMessage: string =
    constant.HTTP_SUCCESS_MESSAGE_MERCHANT_CREATED;
